...
# 2. Fetch the last 365 days of used pairs for deduplication
...
used_rows = supabase_get(
    ...
    f"&date=gte.{one_year_ago}&order=date.desc&limit=400"
)
used_pairs = [
...
prompt = (
    ...
    "2. The pair must NOT appear in the already-used list below (either direction).\n"
    "3. Pairs should be varied: mix famous people, places, historical events, "
    "scientific concepts, pop culture, nature, etc.\n"
    "4. The start and end should NOT be obviously closely related "
    "(e.g. do not pair France with Paris). Make it a meaningful challenge.\n"
    "5. Avoid very obscure articles that most people would not recognise.\n\n"
...
if not isinstance(pairs, list):
    print(f"Expected a JSON array, got: {type(pairs)}", file=sys.stderr)
    sys.exit(1)

# 4. Client-side deduplication as a safety net
used_set = {(p["start"].lower(), p["end"].lower()) for p in used_pairs}
clean_pairs = []
skipped = []
for pair in pairs:
    key = (pair["start_article"].lower(), pair["end_article"].lower())
    if key in used_set:
        skipped.append(pair)
    else:
        used_set.add(key)  # prevent duplicates within the new batch too
        clean_pairs.append(pair)

if skipped:
    print(f"Skipped {len(skipped)} duplicate(s): {skipped}")

if not clean_pairs:
    print("No new unique pairs generated - exiting without insert.", file=sys.stderr)
    sys.exit(1)

# 5. Assign dates and insert into Supabase
puzzles = []
...
try:
    with urllib.request.urlopen(req) as r:
        print(f"Inserted {len(puzzles)} puzzles ({start_date} to {puzzles[-1]['date']})")
        for p in puzzles:
